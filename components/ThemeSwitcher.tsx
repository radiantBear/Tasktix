  ...
  return (
    <Popover isOpen={isOpen}>
      <PopoverTrigger onMouseOver={() => handleMouse(true)} onMouseLeave={() => handleMouse(false)}>
        <Button onClick={() => setTheme(themeIcon == 'dark' ? 'light' : 'dark')} aria-label={`Set ${themeIcon == 'dark' ? 'light' : 'dark'} theme`} variant='ghost' isIconOnly>
          {
            isMounted && themeIcon == 'dark'
              ? <SunFill />
              : <MoonFill />
          }
        </Button>
      ...
      <PopoverContent onMouseOver={() => handleMouse(true)} onMouseLeave={() => handleMouse(false)}>
        <Listbox selectionMode='single' selectedKeys={[ theme || 'system' ]} aria-label='Choose theme'>
