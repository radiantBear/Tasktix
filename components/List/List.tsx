...
export default function List({ startingList, startingTagsAvailable }: { startingList: string, startingTagsAvailable: string }) {
  ...
  return (
    <>
      {list.sections.map(section => <ListSection key={section.id} id={section.id} name={section.name} members={list.members} startingItems={section.items} tagsAvailable={tagsAvailable} deleteSection={deleteListSection.bind(null, section.id)} addNewTag={addNewTag} />)}
