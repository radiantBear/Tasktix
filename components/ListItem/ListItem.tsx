...
export default function ListItem({ item, members, tagsAvailable, hasTimeTracking, hasDueDates, setStatus, setDateCompleted, updateDueDate, updateExpectedMs, deleteItem, addNewTag, reorder }: { item: ListItemModel, members: ListMember[], tagsAvailable: Tag[], hasTimeTracking: boolean, hasDueDates: boolean, setStatus: (status: ListItemModel['status']) => any, setDateCompleted: (date: ListItemModel['dateCompleted']) => any, updateDueDate: (date: Date) => any, updateExpectedMs: (ms: number) => any, deleteItem: () => any, addNewTag: (name: string, color: Color) => any, reorder: () => any }) {  
  ...
  return (
    <Reorder.Item key={item.id} value={item} dragListener={false} dragControls={controls} onDragEnd={reorder} className='border-b-1 border-content3 last:border-b-0'>
      <StaticListItem reorderControls={controls} item={item} members={members} tagsAvailable={tagsAvailable} hasTimeTracking={hasTimeTracking} hasDueDates={hasDueDates} setStatus={setStatus} setDateCompleted={setDateCompleted} updateDueDate={updateDueDate} updateExpectedMs={updateExpectedMs} deleteItem={deleteItem} addNewTag={addNewTag} />
    </Reorder.Item>
  );
}